...
library(GGally)
ggpairs(df_mean, aes(color = diagnosis, alpha = 0.75), lower = list(continuous = "smooth"))
...
generate_boxplot <- function(data, features, variable_type) {
  plot_data <- data.frame(
    value = unlist(data),
    diagnosis = rep(df_clean$diagnosis, each = length(features)),
    feature = rep(features, each = nrow(data))
  )
  
  # Remove _mean, _se, or _worst from variable names
  plot_data$feature <- str_replace_all(plot_data$feature, "_mean", "")
  plot_data$feature <- str_replace_all(plot_data$feature, "_se", "")
  plot_data$feature <- str_replace_all(plot_data$feature, "_worst", "")
  
  plot_data$feature <- str_wrap(plot_data$feature, width = 15)  # Adjust the width as needed
  
  ggplot(plot_data, aes(x = diagnosis, y = value, fill = factor(diagnosis))) +
    geom_boxplot() +
    facet_wrap(~ feature, scales = "free_y", nrow = 1) +
    scale_fill_manual(values = colors, labels = c('Benign', 'Malignant')) +
    xlab("Diagnosis") +
    ylab("Value") +
    ggtitle(variable_type) +  # Add the variable type as the plot title
    theme(legend.position = "top") +
    theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
...
features_mean <- grep("_mean$", names(df_clean), value = TRUE)
features_se <- grep("_se$", names(df_clean), value = TRUE)
features_worst <- grep("_worst$", names(df_clean), value = TRUE)

# Generate boxplots for each set of features
boxplot_mean <- generate_boxplot(df_clean[, features_mean], features_mean, "Mean")
boxplot_se <- generate_boxplot(df_clean[, features_se], features_se, "SE (Standard Error)")
boxplot_worst <- generate_boxplot(df_clean[, features_worst], features_worst, "Worst")
...
# Evaluate Model 1
predictions_rf_1 <- predict(rf_fit_oversampling, newdata = X_test_rf)
confusion_matrix_1 <- confusionMatrix(data = predictions_rf_1, reference = y_test_rf)
precision_1 <- confusion_matrix_1$byClass["Pos Pred Value"]
f1_score_1 <- confusion_matrix_1$byClass["F1"]
auc_roc_1 <- roc(predictions_rf_1, as.numeric(y_test_rf))$auc
...
plot1 <- ggplot(data = df1, aes(x = Reference, y = Prediction)) +
         ...
         theme_minimal() +
         labs(title = "Model 1: RF with Random Oversampling")
print(plot1)
...
plot2 <- ggplot(data = df2, aes(x = Reference, y = Prediction)) +
         ...
         geom_text(aes(label = Freq), color = "black") +
         theme_minimal() +
         labs(title = "Model 2: RF with 70/30 split")
print(plot2)
...
plot3 <- ggplot(data = df3, aes(x = Reference, y = Prediction)) +
         ...
         geom_text(aes(label = Freq), color = "black") +
         theme_minimal() +
         labs(title = "Model 3: RF with K-fold cross-validation")
print(plot3)
```
```{r}
